...
- **工具切换**：控制栏「✋ 拖动视图 / ⚲ 选择节点」切换。`panMode=true` 时左键拖拽平移；`false`（选择工具）时左键点画布节点即跳到该节点所在步骤并激活。任意工具下**双击**画布节点也激活——`dblclick` 监听不判断 `panMode`，避免平移态下点不到节点。

### 坑七：点画布节点高亮了，但底部节点链表不跟随

现象：选择工具下点击（或双击）画布节点，画布上该节点高亮了，但底部节点链表中高亮的仍是上一步的节点——列表没跟着更新。
...
- [x] 工具切换「拖动 / 选择」：选择工具点画布节点即激活；任意工具双击画布节点也激活。
