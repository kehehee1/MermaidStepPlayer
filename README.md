...
- **导出播放文件**：一键生成只含右侧播放界面、已内嵌当前图的独立 HTML（`mermaid-playback.html`），可直接发给他人播放；打开即自动生成当前图，左侧输入被隐藏。
...
5. 点“⤓ 导出播放文件”生成独立的 `mermaid-playback.html`（已内嵌当前图、无左侧输入），可直接发给他人播放；也可在下拉选“GIF 动图”导出动画 GIF。
