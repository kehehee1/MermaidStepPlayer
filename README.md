# Mermaid 流程图 · 深度优先逐步播放器

一个纯前端、零构建的单文件工具：把任意 `flowchart` / `graph` 代码，按**深度优先**顺序逐步揭示节点与边，并可按**分支**（源→汇的简单路径）逐条演示。解析交给开源库 [`mermaid-ast`](https://www.npmjs.com/package/mermaid-ast)（基于 Mermaid 官方 JISON 语法），渲染交给 `mermaid@10`。

## 软件截图

![](https://gitee.com/kehehee1/my-drawing-bed/raw/master/Image/20260820090714459.png)


## 功能特性

- **逐步揭示**：按深度优先顺序逐节点/边高亮，绿色=当前分支路径，橙色脉冲=当前节点。
- **稳定布局（不跳动）**：「全部（深度优先）」逐步播放与「独立分支」演示一致——始终渲染**同一张完整图**，仅用 class 高亮当前分支/节点，不再每步删除未揭示节点重渲染局部图，因此画面不重排、不跳动。
- **分支感知高亮**：「全部（深度优先）」模式下，高亮 = 当前 DFS 路径（当前节点→根）+ 路径上子图容器已揭示的内部节点（上游子图历史保持）+ 下游合并延伸；切换到另一分支时，非当前分支节点（含其所在子图）恢复淡灰，仅高亮当前分支。
- **被跳过公共节点凸显**：「全部（深度优先）」模式下，公共节点（被多条分支共用的汇交点及其上下游，如 `S/G/M/E`）在「不同于其首次出现所在分支」的后续分支中被执行「跳过」时，以**自身形状加紫色虚线描边**凸显（首次正常揭示时不显、分支模式不显）；底部节点链表始终以紫色描边标出这些公共节点，画布本身的绿/灰填充样式始终不变。
- **分支演示**：右侧“分支”下拉框选择某条分支（源→汇的简单路径），整条分支绿色高亮并逐步演示；逐步播放与总览模式一致的“节点弹出 + 连线淡入”过渡效果。
- **环安全**：遇到回边/环（`A → … → A`）不沿其无限遍历，而是检测出来并用**橙色虚线**标为“循环边”，并在总览提示。
- **循环演示与回边反馈**：单环图进入「循环演示」模式，播放循环体一遍并演示「回边返回」（沿回边回到起点，状态栏提示 `↻ 循环返回`）；多环 / 环上带复杂分支的图自动回退到通用 DFS 顺序（分支仍按边序正确跟随），避免单环模型误判。`loopDemo` 标志区分两种模式、状态栏提示如实。回边（循环边）被「走过」时叠加更亮更粗的橙 + 流动虚线反馈，且仅在此步**真正执行的那一条回边**上生效，动效仅持续约 2.6 秒（离开当前节点立即消失）。
- **名称去重**：多分支同名时自动追加 ` #2` `#3`，保证下拉框每项唯一可选。
- **缩放 / 最大化**：滑条、输入框、滚轮三种缩放方式；一键铺满画板。
- **脚本行点击定位**：图已生成后，**点击**左侧源码编辑框某一行（光标落在哪行），画板自动滚动到该行对应节点并**橙色高亮 2 秒**（不改配色、不重渲染，开销极低）；在编辑框内移动鼠标不触发，仅点击时执行。优先定位行内**被描述的目标节点**：如 `A --> B["初始化 report 对象"]` 定位到带描述的 B，而非裸源节点 A；一行多个显式定义时取最后一个（目标），无显式定义时取行末目标节点。
- **缩略导航图**：生成图形后在画板左下角出现固定尺寸的缩略图，展示完整图并提供**视口框**（蓝色边框标出画板当前可见区域，随滚动/缩放实时移动）；点击缩略图任意位置即可把画板滚动到对应位置居中；标题栏可**收起/展开**，最大化模式下自动相对视口固定、避开底部节点链表。
- **底部节点链表**：画布下方用 `>` 串联展示全部节点（深度优先）或当前分支路径，超长名称省略号截断、悬停看全名；点击任一节点即跳到该步并激活，底部列表同步高亮当前节点。
- **画布平移与工具切换**：左键按住拖动画布平移视口（复用 overflow 滚动，仅左键）；控制栏「✋ 拖动视图 / ⚲ 选择节点」切换工具——选择工具下可点选节点内文字（不再平移）；任意工具下双击画布节点即跳到该节点并激活，且底部节点链表始终同步高亮。
- **拷贝节点列表**：控制栏「⧉ 拷贝节点列表」按钮，把当前已运行到的节点（首节点到当前步）以 `>` 连接复制到剪贴板，便于粘贴成路径文本。
- **键盘导航**：`←` 上一步、`→` 下一步、`空格` 播放/暂停、`Esc` 退出最大化 / 关闭指引。
- **操作指引**：内置“操作指引”弹窗，说明全部交互与配色含义。
- **最近浏览历史**：点击“生成步骤并播放”即把当前图记入历史（localStorage 持久化，最多 50 条，最新置顶）；「历史」下拉框以图中第一个节点的真实显示名 + 时间（MM-DD HH:MM）命名，选中或点「浏览」即可重新加载播放；「删除当前」移除正在浏览的记录、「清空」清空全部。
- **导出播放文件**：点“⤓ 导出播放文件”并选格式，可生成两类产物：
  - **独立 HTML 播放文件**：只含右侧播放界面、已内嵌当前图的 `mermaid-playback.html`，可直接发给他人播放；打开即自动生成当前图，左侧输入被隐藏。
  - **GIF 动图**：把逐步揭示的每一步渲染成帧、编码为动画 GIF，可直接贴进 PPT / 文档。GIF 按**完整流程图布局**渲染（各帧尺寸/位置一致、文字大小恒定，未揭示的「是/否」等分支标签不会提前出现），导出尺寸 = 完整图真实尺寸 ×「导出缩放」（默认 2×，可调）。

## 工作流程

```mermaid
flowchart TD
    A[粘贴 flowchart/graph 代码] --> B{图类型判断}
    B -->|非流程图| C[渲染完整图, 不支持逐步揭示]
    B -->|flowchart| D[parseFlowchart 解析为 AST]
    D --> E[findBranches 提取分支 + 回边检测]
    D --> F[buildSteps 深度优先逐步构建]
    E --> G[选择分支 buildBranch]
    F --> H[renderStep 逐步高亮渲染]
    G --> H
    H --> I[交互: 下一步/上一步/自动播放/键盘]
    I --> H
```

## 用法

直接用浏览器打开 `mermaid-step-player.html` 即可（需联网加载 CDN 资源）：

1. 左侧粘贴完整 `flowchart` / `graph` 代码，或选内置示例；
2. 点“生成步骤并播放”，先展示完整流程图；
3. 用“下一步/上一步/自动播放”逐步揭示，或在“分支”下拉框选某条分支演示；
4. 非流程图（时序图、类图等）直接显示完整图，不支持逐步揭示；
5. 点“⤓ 导出播放文件”，先在「格式」下拉选 **独立 HTML 播放文件** 或 **GIF 动图**；导出 GIF 时可用「导出缩放」调节输出尺寸（默认 2×），然后导出：HTML 产物为独立的 `mermaid-playback.html`（已内嵌当前图、无左侧输入），GIF 产物为动画 `mermaid-playback.gif`。
6. 「历史」下拉框记录最近浏览的图（点“生成步骤并播放”时自动记录），选中或点「浏览」可重新加载；「删除当前」删除正在浏览的记录、「清空」清空全部历史。
7. 控制栏「⧉ 拷贝节点列表」可把当前已运行到的节点（首节点到当前步）以 `>` 连接复制到剪贴板，便于粘贴成路径文本。


## 使用说明（流程图）

```mermaid
flowchart TD
    A([用浏览器打开 mermaid-step-player.html]) --> B[左侧粘贴 flowchart/graph 代码<br/>或选内置示例]
    B --> C[点“生成步骤并播放”]
    C --> D[展示完整流程图]
    D --> E{选择演示方式}
    E -->|总览逐步揭示| F["下一步 / 上一步 / 自动播放"]
    E -->|分支演示| G["分支下拉选路径<br/>整条绿色高亮逐步演示"]
    F --> H[缩放 / 最大化 / 键盘导航]
    G --> H
    H --> I{导出格式}
    I -->|独立 HTML| J[导出 mermaid-playback.html<br/>发给他人直接播放]
    I -->|GIF 动图| K[导出 GIF 动图<br/>贴进 PPT / 文档]
    J --> L([完成])
    K --> L
```

##  导出gif效果

![](https://h2cdn.appinn.me/original/3X/1/3/13bfa2ce8fd3419bf8b7ee7da3d1c09a5fc7fe50.gif)

## 测试

分支提取算法（`findBranches`）有配套自测，覆盖：无标签易重名图、带标签复杂图、三路并行、含回边环图、平行同标签去重、多源+合并+回边大图。

```bash
node branch_test.mjs      # 退出码 0=全部通过，1=有失败
```

测试同时是 `findBranches` 的镜像实现（含环检测 `backEdges`、名称去重），改动 HTML 前先跑测试确认不回归。

## 目录结构

```
mermaid-step-player/
├── mermaid-step-player.html   # 单文件播放器（全部逻辑）
├── branch_test.mjs            # 分支算法自测（node 运行）
├── push.bat                   # 一键同时推送 gitee + github
├── README.md
├── diagram/
│   └── 总体设计架构.md          # 架构与算法说明（含 mermaid 图）
└── lesson/                     # 踩坑记录（含 mermaid 对比图）
    ├── 子图顺序与分支高亮踩坑记录.md
    ├── 被跳过公共节点紫色虚线凸显踩坑记录.md
    ├── 含环多循环图播放顺序错乱踩坑记录.md
    └── 回边执行反馈应绑定具体执行边而非节点踩坑记录.md
```

## 已知边界

- 解析/渲染依赖 `esm.sh` 与 `cdn.jsdelivr.net`，需联网。
- 子图（`subgraph`）容器按节点处理，内部边参与 DFS 揭示；子图内部节点执行顺序按「内部入度 0」推导（拓扑序），不依赖解析器 `sg.nodes` 顺序（该顺序可能被打乱）。
- 选根时跳过子图内部节点，避免子图容器不被进入、历史节点丢失高亮。
- 超大图因每步全量重渲染可能偏慢（后续优化方向）。

## 踩坑记录

- 子图执行顺序、选根、分支高亮集合等实战坑点与修正方案，见 [`lesson/子图顺序与分支高亮踩坑记录.md`](lesson/子图顺序与分支高亮踩坑记录.md)（含 Mermaid 对比图）。
- 被跳过公共节点的「凸显时机（分支段切换）」与「公共节点判定（合并点上下游闭包、排除虚线注释边）」踩坑，见 [`lesson/被跳过公共节点紫色虚线凸显踩坑记录.md`](lesson/被跳过公共节点紫色虚线凸显踩坑记录.md)（含 Mermaid 对比图）。
- 含环多循环图（如多个 `H/R/W` 循环）在「全局模式」下决策节点不沿分支走、直接跳到相邻节点的坑，及「单环演示」与「多环回退 DFS」的门控，见 [`lesson/含环多循环图播放顺序错乱踩坑记录.md`](lesson/含环多循环图播放顺序错乱踩坑记录.md)。
- 回边执行反馈从「完全没反应 → 时机混乱 → 精确短暂」的迭代，及「反馈应绑定具体执行边而非节点、动效应限时」的经验，见 [`lesson/回边执行反馈应绑定具体执行边而非节点踩坑记录.md`](lesson/回边执行反馈应绑定具体执行边而非节点踩坑记录.md)。
